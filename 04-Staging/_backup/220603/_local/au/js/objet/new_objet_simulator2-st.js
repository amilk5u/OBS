...
$("body").on("click", ".btn_modelRestBtn", function () {
	...
	if ($objetId == "refrigerator_convertible_M") {
		let mutationDataM = JSON.stringify({ "query": "mutation { addObjetSalesToCart( input: { cartItems: [{ data :{ sku : \"MD075536361\" , qty :1}, zipcode :\"2000\"}] }) {redirectUrl cart { items { product { sku } qty } } } }", "variables": null, "operationName": null })
		console.log(mutationDataM)
